...
请根据电脑的正版授权情况选择正确的版本再下载，否则安装成功后将无法激活。
...
8. 如果其他分区还没有创建，可在此创建分区。注意，GUID 分区表下主分区的数量没有限制，也没有扩展分区、逻辑分区这种极不合理、易丢失数据的糟糕设计。虽然如此，仅仅是从习惯的方面考虑，我们不提倡过多的分区数量，通常以 1 个数据分区为宜，使用文件夹来做好分类。
